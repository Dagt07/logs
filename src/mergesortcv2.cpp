                                   ...
                                   size_t pass_num)
{
    ...
    for (size_t base_run = 0, group = 0; base_run < num_runs; base_run += num_runs_to_merge, ++group)
    {
        ...
        // Archivo de salida para este grupo
        std::string output_filename = "merge_pass" + std::to_string(pass_num) + "_group" + std::to_string(group) + ".bin";
                       ...
                       size_t max_runs_to_merge)
{
    ...
    const std::string initial_runs_file = "runs_initial.bin";

    // Fase 1: Crear ejecuciones ordenadas iniciales
    {
        BufferedFile in_file(input_filename, true, block_size);
        BufferedFile out_file(initial_runs_file, false, block_size);
          ...
          (current_run_files.size() == 1 && current_run_files[0] != output_filename))
    {
        ...
        {
            auto result_files = mergeRuns(run_file, total_elements, run_size, max_runs_to_merge, block_size, pass);
...
size_t findOptimalA(const std::string &input_filename, size_t memory_size, size_t block_size)
{
    ...
    while (left <= right)
    {
        ...
        const std::string temp_input = "temp_input_copy_" + std::to_string(std::chrono::system_clock::now().time_since_epoch().count()) + ".bin";
        {
        ...
        const std::string temp_output = "temp_output_" + std::to_string(std::chrono::system_clock::now().time_since_epoch().count()) + ".bin";
